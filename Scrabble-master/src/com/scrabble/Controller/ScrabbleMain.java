...
public class ScrabbleMain {
	...
	public void challengeButtonPressed() {
		boardWindow.addChallengeButtonActionListener((java.awt.event.ActionEvent evt) -> {
			...
			int response = JOptionPane.showOptionDialog(boardWindow, "Select The word you want to challenge.",
					"Challenge Word", JOptionPane.DEFAULT_OPTION, JOptionPane.PLAIN_MESSAGE, null, words, words[0]);

			if (wordList.challenge(words[response])) {
				JOptionPane.showMessageDialog(boardWindow, words[response] + " is in the wordlist! Challenge Invalid!");
				boardWindow.setChallengeButtonEnabled(false);
		...
		boardWindow.addEndTurnButtonActionListener((java.awt.event.ActionEvent evt) -> {
			...
			for (ScoredWord wordList : gameState.getScoredWords()) {
				for (Space space : wordList) {
					cuvant += space.getAssignedTile().getLetter();
				}
			}
			if (Scrabble.dic.search(cuvant) == null) {
				System.out.println("The word doesn't exist");
				//turnScore = aux;
				currentPlayer.setScore(aux);
				//JFrame parent = new JFrame();
				//JOptionPane.showMessageDialog(parent, "The word doesn't exist");
			}
			// }//end for
			System.out.println("The word is: " + cuvant);
			System.out.println("");
			// }//end for
		...
		boardWindow.addRackButton1ActionListener((java.awt.event.ActionEvent evt) -> {
			...
			if (gameState.getGameState() == GameState.RACK_TILE_SELECTED) {
				...
				boardWindow.setCursor(Toolkit.getDefaultToolkit().createCustomCursor(
						new ImageIcon("src/com/scrabble/view/Scrabble_icon.png").getImage(), new Point(0, 0),
						"custom cursor"));
	...
	public void spaceButtonPressed() {
		boardWindow.addSpaceButtonListActionListener((java.awt.event.ActionEvent evt) -> {
			...
			if (!currentSpace.isOccupied() && gameState.getGameState() == GameState.RACK_TILE_SELECTED) {
				...
				// keep track of spaces played this turn, add selected button's assigned space
				// to played list
